...
//		AddMessage2Log(print_r($_REQUEST, true), "");
		if($_REQUEST['PERSON_TYPE']==1) $ufType=4;
		if($_REQUEST['PERSON_TYPE']==2) $ufType=5;
	...
	protected function initOrderFields(Order $order)
	{
		...
		$order->setField("ADDITIONAL_INFO", $this->arUserResult['ORDER_DESCRIPTION']);
		$order->setField("COMMENTS", $this->arUserResult['ORDER_DESCRIPTION']);
		...
		if ($request->isPost())
		{
			...
			if (empty($arUserResult["PERSON_TYPE_OLD"]) || $arUserResult["PERSON_TYPE_OLD"] == $arUserResult["PERSON_TYPE_ID"])
			{
				...
				if (strlen($request->get('ORDER_DESCRIPTION')) > 0)
				{
					...
					$arUserResult["ADDITIONAL_INFO"]=$request->get('ORDER_DESCRIPTION');
					$arUserResult["COMMENTS"]=$request->get('ORDER_DESCRIPTION');
				}
			...
			foreach ($modifiedFields as $field => $value)
			{
				switch ($field)
				{
					...
					case 'ORDER_DESCRIPTION':
						...
						$order->setField('ADDITIONAL_INFO', $value);
						$order->setField('COMMENTS', $value);
						break;
