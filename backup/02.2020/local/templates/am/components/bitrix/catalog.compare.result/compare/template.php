...
if($ar_res = $res->GetNext())
  // print_r();
echo "<center><img width='auto' height='150' src='".CFile::GetPath($ar_res['PREVIEW_PICTURE'])."'/>";?>
						<a href="<?=$arElement["DETAIL_PAGE_URL"]?>"><?
							// print_r($arElement);
							// для img

						?><b><?=$arElement[$code]?></b></a></center>
...
if (!empty($arResult["SHOW_OFFER_PROPERTIES"]))
{
	...
	<tr>
		...
		?>
		<td align=center>
			<a class="btn btn-main btn-hover-white btn-no-icon" onclick="CatalogCompareObj.MakeAjaxAction('<?=CUtil::JSEscape($arElement['~DELETE_URL'])?>');" href="javascript:void(0)"><span><?=GetMessage("CATALOG_REMOVE_PRODUCT")?></span></a>
		</td>
		<?
		}
		unset($arElement);
